...
# Drop the first two columns from y_train and assign them to a separate DataFrame
additional_features = y.iloc[:, :2]

# Drop the first two columns from y_train
y = y.iloc[:, 2:]

# Concatenate additional_features with x_train
...
cv_scores = cross_val_score(rf_classifier, x_train, np.ravel(y_train), cv=5, scoring='accuracy')
